...
namespace Intwenty.Data
{
    ...
    public class DefaultDataManager : IDataManager
    {
        ...
        public DefaultDataManager(IIntwentyModelService modelservice, IOptions<IntwentySettings> settings)
        {
        ...
        public virtual OperationResult Validate(ApplicationModel model, ClientStateInfo state)
        {
        ...
        public virtual OperationResult GetLatestVersionById(ApplicationModel model, ClientStateInfo state)
        {
            if (state == null)
                return new OperationResult(false, MessageCode.SYSTEMERROR, "state was null when executing DefaultDbManager.GetLatestVersionById.", 0, 0);
            if (state.Id < 0)
                return new OperationResult(false, MessageCode.SYSTEMERROR, "Id is required when executing DefaultDbManager.GetLatestVersionById.", 0, 0);
            if (state.ApplicationId < 0)
                return new OperationResult(false, MessageCode.SYSTEMERROR, "ApplicationId is required when executing DefaultDbManager.GetLatestVersionById.", 0, 0);


            return GetLatestVersion(model,state);
        }

        public virtual OperationResult GetLatestVersionByOwnerUser(ApplicationModel model, ClientStateInfo state)
        {
            if (state == null)
                return new OperationResult(false, MessageCode.SYSTEMERROR, "state was null when executing DefaultDbManager.GetLatestVersionByOwnerUser.", 0, 0);
            if (string.IsNullOrEmpty(state.OwnerUserId))
                return new OperationResult(false, MessageCode.SYSTEMERROR, "OwnerUserId is required when executing DefaultDbManager.GetLatestVersionByOwnerUser.", 0, 0);
            if (state.ApplicationId < 0)
                return new OperationResult(false, MessageCode.SYSTEMERROR, "ApplicationId is required when executing DefaultDbManager.GetLatestVersionByOwnerUser.", 0, 0);

            var t = FetchLatestIdByOwnerUser(model, state);
            if (t.Id < 1)
        ...
        protected virtual OperationResult GetLatestVersion(ApplicationModel model, ClientStateInfo state)
        {
        ...
        public virtual OperationResult GetVersion(ApplicationModel model)
        {
            throw new NotImplementedException();
        }

        public virtual OperationResult Save(ApplicationModel model, ClientStateInfo state)
        {

            if (state == null)
                return new OperationResult(false, MessageCode.SYSTEMERROR, "No client state found when performing save application.", 0, 0);

            var result = new OperationResult(true, MessageCode.RESULT, string.Format("Saved application {0}", model.Application.Title), state.Id, state.Version);
            ...
            try
            {
                ...
                BeforeSave(model,state);

                if (state.Id < 1)
                {
                    state.Id = GetNewSystemID(model.Application.Id, "APPLICATION", model.Application.MetaCode, state);
                    result.Status = LifecycleStatus.NEW_NOT_SAVED;
                    state.Version = CreateVersionRecord(model, state);
                    BeforeSaveNew(model,state);
                    InsertMainTable(model,state);
                    InsertInformationStatus(model, state);
                    HandleSubTables(model,state);
            ...
            catch (Exception ex)
            {
                result.Messages.Clear();
                result.IsSuccess = false;
                result.AddMessage(MessageCode.USERERROR, string.Format("Save application {0} failed", model.Application.Title));
                result.AddMessage(MessageCode.SYSTEMERROR, ex.Message);
            }
            finally
            {
        ...
        public virtual OperationResult DeleteById(ApplicationModel model, ClientStateInfo state)
        {
            if (state == null)
                return new OperationResult(false, MessageCode.SYSTEMERROR, "No client state found when performing DeleteById(state).", 0, 0);

            if (state.Id < 1)
                return new OperationResult(false, MessageCode.SYSTEMERROR, "No state.Id found when performing DeleteById(state).", 0, 0);

            var result = new OperationResult(true, MessageCode.RESULT, string.Format("Deleted application {0}", model.Application.Title), state.Id, 0);
        ...
        public virtual OperationResult DeleteById(ApplicationModel model, int id, string dbname)
        {

            if (id < 1)
                return new OperationResult(false, MessageCode.SYSTEMERROR, "No id found when performing DeleteById(id, dbname).", 0, 0);
            if (string.IsNullOrEmpty(dbname))
                return new OperationResult(false, MessageCode.SYSTEMERROR, "No dbname found when performing DeleteById(id, dbname).", 0, 0);
            ...
            catch (Exception ex)
            {
                ...
                result.Messages.Clear();
                result.IsSuccess = false;
                result.AddMessage(MessageCode.USERERROR, string.Format("DeleteById(id, dbname) in application {0} failed", model.Application.Title));
                result.AddMessage(MessageCode.SYSTEMERROR, ex.Message);
            }
            finally
            {
        ...
        public virtual OperationResult GetList(ApplicationModel model, ListRetrivalArgs args)
        {
            return GetListInternal(model, args, false, true, true);
        }

        public virtual OperationResult GetListByOwnerUser(ApplicationModel model, ListRetrivalArgs args)
        {
            return GetListInternal(model, args, true, true, true);
        }

        public virtual OperationResult GetListByOwnerUser(ApplicationModel model, string owneruserid)
        {
            ...
            if (Settings.DefaultConnectionDBMS == DBMS.PostgreSQL)
                return GetListInternal(model, new ListRetrivalArgs() { ApplicationId = model.Application.Id, OwnerUserId = owneruserid }, true, true, false);
            else
                return GetListInternal(model, new ListRetrivalArgs() { ApplicationId = model.Application.Id, OwnerUserId = owneruserid }, true, false, false);


        }

        public virtual OperationResult GetList(ApplicationModel model)
        {
    
            //Postgresql must always use columns in order to set correct case in jsonnames
            if (Settings.DefaultConnectionDBMS == DBMS.PostgreSQL)
                return GetListInternal(model,new ListRetrivalArgs() { ApplicationId = model.Application.Id }, false, true, false);
            else
                return GetListInternal(model,new ListRetrivalArgs() { ApplicationId = model.Application.Id }, false, false, false);
        }
        ...
        public virtual OperationResult GetVersionListById(ApplicationModel model, ClientStateInfo state)
        {
            throw new NotImplementedException();
        }

        public virtual OperationResult GetVersion(ApplicationModel model, ClientStateInfo state)
        {
            throw new NotImplementedException();
        }
        ...
        /// </summary>
        public virtual OperationResult GetApplicationValueDomains(ApplicationModel model)
        {
           
            var sb = new StringBuilder();
            var result = new OperationResult(true, MessageCode.RESULT, string.Format("Fetched domains used in ui for application {0}", model.Application.Title), 0, 0);
            ...
            catch (Exception ex)
            {
                result.Messages.Clear();
                result.IsSuccess = false;
                result.AddMessage(MessageCode.USERERROR, string.Format("Fetch valuedomains for application {0} failed", model.Application.Title));
                result.AddMessage(MessageCode.SYSTEMERROR, ex.Message);
                result.Data = "[]";
        ...
        {
            var sb = new StringBuilder();
            var result = new OperationResult(true, MessageCode.RESULT, "Fetched all value domins", 0, 0);
        ...
        public virtual OperationResult GetDataView(ListRetrivalArgs args)
        {
        ...
        public virtual OperationResult GetDataViewRecord(ListRetrivalArgs args)
        {
            var sql = "";
            var result = new OperationResult(true, MessageCode.RESULT, "Fetched dataview record", 0, 0);
        ...
        protected virtual void BeforeSave(ApplicationModel model, ClientStateInfo data)
        {

        }

        protected virtual void BeforeSaveNew(ApplicationModel model, ClientStateInfo data)
        {

        }

        protected virtual void BeforeSaveUpdate(ApplicationModel model, ClientStateInfo data)
        {

        }

        protected virtual void AfterSave(ApplicationModel model, ClientStateInfo data)
        {
        ...
        protected virtual void SetParameters(List<ApplicationValue> paramlist, List<IIntwentySqlParameter> parameters)
        {
        ...
        protected virtual int GetNewSystemID(int applicationid, string metatype, string metacode, ClientStateInfo state)
        {
        ...
        protected virtual DateTime GetApplicationTimeStamp()
        {
        ...
        protected virtual void InsertMainTable(ApplicationModel model, ClientStateInfo state)
        {
        ...
        protected virtual void HandleSubTables(ApplicationModel model, ClientStateInfo state)
        {
            foreach (var table in state.Data.SubTables)
            {
                if (!table.HasModel)
                ...
                foreach (var row in table.Rows)
                {
                    ...
                    {
                        InsertTableRow(model, row, state);
        ...
        protected virtual void UpdateMainTable(ApplicationModel model, ClientStateInfo state)
        {
            var paramlist = new List<ApplicationValue>();
        ...
        protected virtual void InsertTableRow(ApplicationModel model, ApplicationTableRow data, ClientStateInfo state)
        {
            var paramlist = new List<ApplicationValue>();

            var rowid = GetNewSystemID(model.Application.Id, DatabaseModelItem.MetaTypeDataTable, data.Table.Model.MetaCode, state);
            if (rowid < 1)
        ...
        protected virtual void UpdateTableRow(ApplicationTableRow data, ClientStateInfo state)
        {
            var paramlist = new List<ApplicationValue>();
        ...
        protected virtual int CreateVersionRecord(ApplicationModel model, ClientStateInfo state)
        {
            int newversion = 0;
        ...
        protected virtual OperationResult FetchLatestIdByOwnerUser(ApplicationModel model, ClientStateInfo state)
        {
            var result = new OperationResult(true, MessageCode.RESULT, string.Format("Fetched latest id for application {0} for Owner {1}", model.Application.Title, state.OwnerUserId), 0, 0);
            ...
            try
            {
                ...
                if (maxid != null && maxid != DBNull.Value)
                {
                    ...
                    {
                        var fail = new OperationResult(false, MessageCode.USERERROR, string.Format("Latest id for application {0} for Owner {1} could not be found", model.Application.Title, state.OwnerUserId), 0, 0);
                        return fail;
                    }
            ...
            catch (Exception ex)
            {
                ...
                result.Messages.Clear();
                result.IsSuccess = false;
                result.AddMessage(MessageCode.USERERROR, string.Format("Fetched latest id for application {0} for Owner {1} failed", model.Application.Title, state.OwnerUserId));
                result.AddMessage(MessageCode.SYSTEMERROR, ex.Message);
                result.Data = "{}";
        ...
        protected virtual void InsertInformationStatus(ApplicationModel model, ClientStateInfo state)
        {
            var m = new InformationStatus() { Id = state.Id, ApplicationId= model.Application.Id, 
                                                  ChangedBy= state.UserId, ChangedDate = DateTime.Now, CreatedBy = state.UserId, 
                                                  MetaCode = model.Application.MetaCode, OwnedBy = state.OwnerUserId, 
        ...
        protected virtual void UpdateInformationStatus(ClientStateInfo state)
        {
